...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(elm => elm.CP > 200).map(elem => elem.monster + ", ").reduce((acc, cur) => acc + cur)
// console.log("Strongest: " + myStrongest)
...
// CODE HERE
const priceAfterTax = orders.map(element => {
  const prices = []
  prices.push(element.price * (1+element.tax))
  return prices
})
